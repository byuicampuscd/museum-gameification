...
$(document).ready(function () {
    ...
    function buildTooltipHTML(link) {
        ...
        if (link.homePage)
            link.filePath = 'https://byui.brightspace.com/d2l/home/' + ou;
        else
    ...
    links.forEach(function (link) {
        ...
        if (window.location.href.split('/')[window.location.href.split('/').length - 1].includes(link.filePath.replace('./', ''))) {
        ...
        if (document.querySelector(`#${link.selector}`) != null) {
            document.querySelector(`#${link.selector}`).parentElement.href.baseVal = link.filePath;
        }
    });
});